(function () {
    ...
    function initToolbar() {
        ...
        addIconButton(toolbar, "earth.png", test, "Старт кода")
        below = div()
    ...
    async function printDiaNodes(data) {
        items = data.items;
        ...
        if (currentItem.type == 'pause' ) {
            await sleep(currentItem.content);
        }
        if (typeof window !== 'undefined') {
            // Оберните код в функцию, чтобы он выполнялся после загрузки документа
            window.addEventListener('DOMContentLoaded', () => {
              const links = document.querySelectorAll('a[href*="DrakonWidget"]');
          
              links.forEach(link => {
                const shadowElement = document.createElement('div');
                shadowElement.classList.add('shadow-mode');
          
                const dynamicObjects = link.querySelectorAll('.dynamic-object');
                dynamicObjects.forEach(object => object.remove());
          
                shadowElement.appendChild(link.cloneNode(true));
                document.body.appendChild(shadowElement);
              });
          
              // Удаление динамических объектов при завершении работы
          ...
          if (currentItem.content) {
              ...
              console.log(`Link: ${currentItem.link || ''}`);
              console.log();
              await sleep(1000);
        }
 if (typeof window !== 'undefined') {
    ...
    function  test() {
        //alert("hytrwqytsfqhgjbsavmb")
        var current = localStorage.getItem("current-diagram")
        var diagram = JSON.parse(localStorage.getItem(current))
        printDiaNodes(diagram)
        console.log('*******************************************')
        console.log(diagram)
    }
